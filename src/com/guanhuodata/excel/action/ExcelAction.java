...
public class ExcelAction implements Action {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		}else if("showExcelPage".equals(type)){
			showExcelPage(request, response);
		}
	...
	//获取上传的excel报表信息
		...
		private void uploadExcel(HttpServletRequest request, HttpServletResponse response) throws IOException {
	        ...
	        excelBean.setDirectName("用户");
	        excelBean.setClick(3);
	        excelBean.setClickOutput_15(3);
	        excelBean.setAllowSpreadTheme("斐夷所丝爆发期");
	        String json = JsonUtil.makeJson(excelBean);
	        	...
	        	for(FileItem item:list){
		        	...
		        	//对传入的非简单的字符串进行处理,比如:二进制的图片、电影等
		        		...
		        		if(!(".xls".equals(fileName.substring(fileName.lastIndexOf("."), fileName.length()))) && !(".xlsx".equals(fileName.substring(fileName.lastIndexOf("."), fileName.length())))){
		        			LOG.info("Upload File is Not Excel.");
			        		...
			        		session.setAttribute("filePath", savePath);
			        		//我们自己来写一个
			        		/*InputStream in = item.getInputStream();
			        		FileOutputStream out = new FileOutputStream(filePath);
			        		int length = 0;
			        		byte[] buf = new byte[1024];
			        		System.out.println("获取上传文件总大小：" + item.getSize());
			        		//in.read(buf);
		...
		private void showExcelPage(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			LOG.info("show excel list page.");
			request.getSession().getServletContext().getRequestDispatcher("/pages/showExcelList/showUploadedExcelList.html").forward(request, response);
		}
}
